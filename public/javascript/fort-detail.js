...
async function loadFortData(fortId) {
  console.log('Loading fort data for ID:', fortId);
  
  try {
    // First try to get fort data from API
    const res = await fetch(`/api/forts/${fortId}`);
    console.log('API response status:', res.status);
    
    if (res.ok) {
      const fort = await res.json();
      console.log('Fort data from API:', fort);
      currentFortId = fortId;
      updateContentFromAPI(fort);
      
      const heroSection = document.getElementById('fortHero');
      if (heroSection) {
        heroSection.style.backgroundImage = `linear-gradient(rgba(40,20,10,0.7), rgba(40,20,10,0.7)), url('${fort.image_url || '/images/default-fort.jpg'}')`;
      }
      
      const fortImage = document.getElementById('fortImage');
      if (fortImage) {
        fortImage.src = fort.image_url || '/images/default-fort.jpg';
        fortImage.alt = fort.name;
      }
      return;
    } else {
    ...
    console.log('API not available, trying static data...', error);
  }

  // Fallback to static data if API fails
  console.log('Trying static data for fortId:', fortId);
  const fort = fortsData[fortId];
  if (!fort) {
    console.log('Fort not found in static data');
    document.getElementById('fortName').textContent = 'Fort not found';
    return;
  }

  console.log('Using static data:', fort);
  currentFortId = fortId;
  ...
  const fortImage = document.getElementById('fortImage');
  if (fortImage) {
    fortImage.src = `images/${fortId}.jpg`;
    fortImage.alt = fortData.name;
  }
}

function updateContentFromAPI(fort) {
  console.log('Updating content with fort data:', fort);
  
  // Update basic information
  const fortName = document.getElementById('fortName');
  if (fortName) fortName.textContent = fort.name || 'Unknown Fort';
  
  const fortLocation = document.getElementById('fortLocation');
  if (fortLocation) fortLocation.textContent = fort.location || 'Location not specified';
  
  const baseVillage = document.getElementById('baseVillage');
  if (baseVillage) baseVillage.textContent = fort.base_village || 'Not specified';
  
  const difficulty = document.getElementById('difficulty');
  if (difficulty) {
    difficulty.textContent = fort.difficulty || 'Not specified';
    // Update difficulty badge
    const difficultyClass = (fort.difficulty || '').toLowerCase();
    difficulty.className = `stat-value difficulty-badge difficulty-${difficultyClass === 'easy' ? 'easy' : difficultyClass === 'moderate' || difficultyClass.includes('moderate') ? 'moderate' : 'difficult'}`;
  }
  
  const height = document.getElementById('height');
  if (height) height.textContent = fort.height || 'Not specified';
  
  const fortDescription = document.getElementById('fortDescription');
  if (fortDescription) fortDescription.textContent = fort.description || 'No description available';
  
  // Set default values for fields not in API
  const historyContent = document.getElementById('historyContent');
  if (historyContent) historyContent.textContent = fort.history || 'Historical information not available for this fort.';
  
  const duration = document.getElementById('duration');
  if (duration) duration.textContent = fort.duration || 'Not specified';
  
  const distance = document.getElementById('distance');
  if (distance) distance.textContent = fort.distance || 'Not specified';
  
  const bestTime = document.getElementById('bestTime');
  if (bestTime) bestTime.textContent = fort.bestTime || 'Year round';
  
  const grade = document.getElementById('grade');
  if (grade) grade.textContent = fort.grade || fort.difficulty || 'Not specified';
  
  const routeText = document.getElementById('routeText');
  if (routeText) routeText.textContent = fort.routeDescription || 'Route information not available.';
  
  const baseVillageDetails = document.getElementById('baseVillageDetails');
  if (baseVillageDetails) baseVillageDetails.textContent = fort.baseVillageDetails || 'Base village details not available.';

  // Update lists with default values since API doesn't have detailed lists
  updateLists('structures', fort.structures || ['Fort walls and bastions', 'Main entrance gate', 'Watchtowers']);
  updateLists('waterSources', fort.waterSources || ['Natural water sources', 'Rainwater collection']);
  updateLists('temples', fort.temples || ['Temple ruins', 'Religious structures']);
  updateLists('viewpoints', fort.viewpoints || ['Main fort plateau', 'Bastion viewpoints']);
  updateLists('attractions', fort.attractions || ['Historical ruins', 'Scenic views', 'Fort architecture']);
  updateLists('photographySpots', fort.photographySpots || ['Fort walls', 'Scenic landscapes', 'Historical structures']);
  updateLists('wildlife', fort.wildlife || ['Local bird species', 'Wildlife in surrounding area']);
  updateLists('facilities', fort.facilities || ['Basic facilities', 'Parking available']);
}
